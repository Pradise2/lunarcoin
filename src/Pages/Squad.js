...
const Squad = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const squadResponse = await axios.get(`https://lunarapp.thelunarcoin.com/backend/api/squad/${userId}`);
  ...
  const handleClaim = async (userId) => {
    ...
    const totalBalance = Number(newTotalSquad);
  ...
  const displayTotalBalance = Number(userSquad?.claimedReferral || 0).toLocaleString('en-US', {
          ...
          {squads.length > 0 ? (
            squads.map((referral, index) => (
              <div key={index} className="flex font-normal text-sm justify-between items-center px-3">
                ...
                <p className="text-primary">{referral.referralEarning} <span className="text-fin">LAR</span></p>
              </div>
            ))
          ) : (
            <p className="text-center text-zinc-400 text-sm">No referrals yet. Share your link and start earning!</p>
          )}
        </div>
      </div>
      <div className="relative bg-sinc bg-opacity-10 p-4 rounded-xl w-full max-w-md space-y-2">
        <button
          className={`px-4 py-2 rounded-xl ${copied ? 'bg-golden-moon text-white ' : 'bg-zinc-500 text-white'}`}
